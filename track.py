...
def run(
    ...
    for frame_idx, (path, im, im0s, vid_cap, s) in enumerate(dataset):
        ...
        for i, det in enumerate(pred):  # detections per image
            ...
            if det is not None and len(det):
                ...
                if len(outputs[i]) > 0:
                    for j, (output, conf) in enumerate(zip(outputs[i], confs)):
                        ...
                        #获取目标ID值
                        int_id = int(output[4])
                        #判断一下如果现在的列表长度小于新产生的ID值，说明出现了一个或多个新目标，把他加入到列表中。
                        if (len(list_sign) < int_id):
                            #这个是用来控制循环的变量而已
                            listsign_i = 0
                            #有时会同时出现多个人，这个变量是保存新增了多少个人
                            temp=int_id-len(list_sign)
                            while listsign_i < temp:
                                list_sign.append(0)
                                listsign_i += 1;
                        #同上，加入ID的列表，上面是加入ID标志的列表
                        if (len(list_id) < int_id):
                         ...
                         #top是指的检测框上边缘线的位置，h是指的框的高度，top+h实际指的是检测框下边缘线的位置，
            ...
            # Save results (image with detections)是否保存结果
            if save_vid:
